...
export function getObjectiveSection(experiments?: Record<string, boolean>): string {
	// Check if task cards are enabled
	const taskCardsEnabled = experiments?.[EXPERIMENT_IDS.TASK_CARDS] === true

	// Base objective section that's always included
	const baseObjective = `====
...
5. The user may provide feedback, which you can use to make improvements and try again. But DO NOT continue in pointless back and forth conversations, i.e. don't end your responses with questions or offers for further assistance.`
	...
	// Additional task card specific objectives
...
1. Analyze the user's task and set clear, achievable goals to accomplish it. Prioritize these goals in a logical order.
2. Begin by creating a minimal task card with the core task details. Add initial steps focused on information gathering before attempting implementation.
...
4. Progressively update the task card as you gather information and make progress - don't try to fill it with speculation or guesses.
...
6. Once you've completed the user's task, you must use the attempt_completion tool to present the result of the task to the user. Be sure to update the task card with your final progress. You may also provide a CLI command to showcase the result of your task; this can be particularly useful for web development tasks, where you can run e.g. \`open index.html\` to show the website you've built.
